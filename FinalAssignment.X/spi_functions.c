...
void print_float_digits(int row, int start, double num){ // 5 cells of the LCD are printed by this function
    place_cursor(row,start);
    
    // DETERMINE THE SIGN OF THE FLOAT PASSED AS INPUT, PRINT IT AND EVALUATE ITS ABSOLUTE VALUE
    if (num >= 0){
        num = num; // evaluate the absolute value of num and store it in the same variable
        while(SPI1STATbits.SPITBF == 1); // wait until not full -> if this bit is 1 the buffer is full
        SPI1BUF = '+';  // print '+' on the LCD
    ...
    // MOVE THE COMMA TWO CELLS TO THE RIGHT
    // float nearest = roundf(num * 100) / 100;  /* Result: 37.78 */
    num = num*100;
    int int_num = (int) num;
    // WRITE THE ABSOLUTE VALUE OF THE INTEGER PASSED AS INPUT, BY PRINTING ON THE LCD ONE DIGIT AT A TIME AS A CHARACTER
    
    while(SPI1STATbits.SPITBF == 1); // wait until not full -> if this bit is 1 the buffer is full
    SPI1BUF = (unsigned char)(int_num/1000) + '0'; // extract the thousands digit from the num
    
    while(SPI1STATbits.SPITBF == 1); // wait until not full -> if this bit is 1 the buffer is full
    SPI1BUF = (unsigned char)((int_num%1000)/100) + '0'; // extract the hundreds digit from the num
    
    while(SPI1STATbits.SPITBF == 1); // wait until not full -> if this bit is 1 the buffer is full
    SPI1BUF = '.'; // print a dot
    
    while(SPI1STATbits.SPITBF == 1); // wait until not full -> if this bit is 1 the buffer is full
    SPI1BUF = (unsigned char)(((int_num%100)/10)) + '0'; // extract the tens digit from the num
    
    while(SPI1STATbits.SPITBF == 1); // wait until not full -> if this bit is 1 the buffer is full
    SPI1BUF = (unsigned char)((int_num%10)/1) + '0'; // extract the units digit from the num
